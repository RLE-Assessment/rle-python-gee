...
import ee
from rle.gee.auth import check_authentication, is_authenticated, print_authentication_status


class TestAuthenticationFunctions:
    """Test suite for Earth Engine authentication functions."""

    @patch('rle.gee.auth.ee.Initialize')
    @patch('rle.gee.auth.ee.data.getAssetRoots')
    ...
    @patch('rle.gee.auth.check_authentication')
